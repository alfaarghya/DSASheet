...
class BubbleSort {
    ...
    BubbleSort() { // when.....we don't have the data to pass.....we want to pass the data at run
                   // time
        inputData();
    }
    /*---- ----*/
    ...
    public void bubbleSortAscendingOrder() { // TC -> O(n^2)
        ...
        for (int i = 0; i < n - 1; i++) { // run a loop >> 0 to 2nd last element
            ...
            for (int j = 0; j < n - 1 - i; j++) { // run a loop >> 0 to n-1-i
                if (arr[j] > arr[j + 1]) { // when current[j] element is greater than it's next element[j+1] push that
                                           // after the element[j+1]......it means if we have 4,3,2,1 >> we need to push
                                           // 4 after 3
                    // -- swapping --//
                    int temp = arr[j];
                    arr[j] = arr[j + 1];
    ...
    public void bubbleSortDescendingOrder() { // TC -> O(n^2)
        ...
        for (int i = 0; i < n - 1; i++) { // run a loop >> 0 to 2nd last element
            int swap = 0; // it tracks how many swapping done
            for (int j = 0; j < n - 1 - i; j++) { // run a loop >> 0 to n-1-i
                if (arr[j] < arr[j + 1]) { // when current[j] element is less than it's next element[j+1] push that
                                           // after the element[j+1]......it means if we have 1,2,3,4 >> we need to push
                                           // 1 after 2
                    // -- swapping --//
                    int temp = arr[j];
                    arr[j] = arr[j + 1];
...
public class SA01_BubbleSort {
    public static void main(String[] args) {
        ...
        /*
         * System.out.println("--------------------------------------");
         * BubbleSort bs1 = new BubbleSort(); //use this when u want to give the data at
         * run time
         * System.out.print("Original array >> ");
         * bs1.printArray();
